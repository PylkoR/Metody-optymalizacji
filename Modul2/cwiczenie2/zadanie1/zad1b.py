...
a, L, C, N = data['a'].item(), data['L'].item(), data['C'].item(), data['N'].item()
...
# Cel: Minimalizacja pola
objective = cp.Minimize(h * cp.sum(y))

# Ograniczenia podstawowe, długości i krzywizny
constraints = [
    y[0] == 0, y[N] == 0,
    y[fixed_idx] == y_fixed_all[fixed_idx],
    y >= 0, # Dodatkowy warunek nieujemności 
    cp.sum([cp.norm(cp.vstack([h, y[i+1] - y[i]])) for i in range(N)]) <= L,
    cp.abs(y[2:] - 2*y[1:-1] + y[:-2]) <= C * h**2
]

prob = cp.Problem(objective, constraints)
prob.solve()

print(f"Minimalne pole przy y >= 0 (1b): A = {prob.value:.4f}") # Odp: 0.1890 
